...
import { deleteBlog, updateBlog } from '../redux/blogs/blogsActions';
...
function SingleBlog({ id, title, content, image }) {
    ...
    const editHandler = (e) => {
        e.preventDefault();
        if (editTitle === '' || editContent === '' || editImage === '') {
            setErrors(true);
            ...
            setErrors(false);
            setErrorMessage('');
            const blog = { id, title: editTitle, content: editContent, image: editImage };
